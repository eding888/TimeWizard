...
userRouter.get('/friend/:username', async (request, response) => {
    if (!request.user) {
        return response.status(401).json({ error: 'User/token not found' });
    }
...
userRouter.post('/sendFriendRequest/:username', async (request, response) => {
    const username = request.params.username;
    if (!request.user) {
        return response.status(401).json({ error: 'User/token not found' });
    }
...
userRouter.post('/acceptFriendRequest/:username', async (request, response) => {
    const username = request.params.username;
    const user = request.user;
    if (!user) {
        return response.status(401).json({ error: 'User/token not found' });
...
userRouter.post('/rejectFriendRequest/:username', async (request, response) => {
...
userRouter.post('/removeFriend/:username', async (request, response) => {
    const username = request.params.username;
    const user = request.user;
    if (!user) {
        return response.status(401).json({ error: 'User/token not found' });
